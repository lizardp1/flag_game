...
DEFAULT_OUT = ROOT / "runs" / "vision_test"
DEFAULT_MODELS = ["gpt-4o", "gpt-5.4-2026-03-05"]
...
def figure_output_dirs(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    if figure_dir is None:
        figure_dir = args.out / "figures" if is_standard_out else args.out / "figures"
    if data_dir is None:
        data_dir = args.out / "data" if is_standard_out else args.out / "data"
...
def run_figure_script(
    ...
    *,
    ...
    script = ROOT / "paper" / "figures" / "code" / "make_flag_visual_only_audit_visuals.py"
    command = [
        sys.executable,
        str(script),
        "--source",
