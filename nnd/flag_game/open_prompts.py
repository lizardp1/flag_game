...
def _base_header(*, memory_lines: list[str]) -> str:
    return (
...
def _interaction_schema(m: int) -> str:
    ...
    if m == 2:
        return 'Output JSON exactly: {"country":"<one country>","clue":"<short phrase>"}'
    if m == 3:
        return 'Output JSON exactly: {"country":"<one country>","reason":"<one sentence>"}'
...
def _task_text(
    *,
    ...
    lines = [f"{_base_header(memory_lines=memory_lines)}"]
