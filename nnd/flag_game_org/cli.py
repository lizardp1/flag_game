...
def run(
    ...
    config_obj = load_org_flag_game_config(config)
    if override:
    ...
    result = run_org_flag_game_experiment(config_obj, out_dir=out, seed=seed)
...
def batch(
    ...
    results = run_org_flag_game_batch(config_obj, out_dir=out, seeds=seeds)
...
def mix_sweep(
    ...
    seeds = list(range(start_seed, start_seed + num_seeds))
    result = run_org_flag_game_mix_sweep(
        config_obj,
        out_dir=out,
        seeds=seeds,
...
def role_mix(
    ...
    prestige_model: str = typer.Option("gpt-5.4", "--prestige-model"),
    start_seed: int = typer.Option(0, "--start-seed"),
    num_seeds: int = typer.Option(20, "--num-seeds"),
    backend: str | None = typer.Option(None, "--backend"),
    agent_workers: int | None = typer.Option(None, "--agent-workers"),
    seed_workers: int | None = typer.Option(None, "--seed-workers"),
    override: list[str] | None = typer.Option(None, "--override"),
    ...
    if backend is not None:
        ...
        config_obj = config_obj.model_copy(update={"backend": "openai"})
    seeds = list(range(start_seed, start_seed + num_seeds))
    result = run_org_flag_game_role_mix_comparison(
        config_obj,
        out_dir=out,
        seeds=seeds,
        comparison_model=comparison_model,
    ...
    console.print(f"Role-mix comparison complete. Output saved to {out}")
    console.print(f"Conditions run: {len(result['summary'])}")
    console.print("Wrote comparison summaries:")
    console.print(f"- {out / 'role_mix_summary.csv'}")
    console.print(f"- {out / 'role_mix_condition_results.csv'}")
    console.print(f"- {out / 'role_mix_report.md'}")
