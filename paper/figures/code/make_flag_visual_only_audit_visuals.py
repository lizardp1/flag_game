...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/flag-game-mplconfig")
os.environ.setdefault("XDG_CACHE_HOME", "/tmp/flag-game-cache")
...
DEFAULT_SOURCE_CSV = (
    BUNDLE_ROOT / "data" / "raw" / "visual_only_paired_crop_audit" / "results.csv"
)
DEFAULT_FIGURE_DIR = BUNDLE_ROOT / "generated"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional fixed evaluation slice for the stacked category bars. "
            "This lets later qualitative follow-ups remain available without "
            "changing the n=100 audit denominator."
        ),
    )
    parser.add_argument(
        "--figure-dir",
...
def resolve_crop_path(value: Any, *, source: Path) -> Path | None:
    ...
    if path.is_absolute():
        # Prefer packaged stimuli over historical machine-specific paths.
        bundled = source.parent / "artifacts" / path.name
        if bundled.exists():
            return bundled
        return path
    ...
    if bundled.exists():
        return bundled
    return BUNDLE_ROOT.parent.parent / path
...
def draw_visual_read_category_panel(
    ...
    *,
    ...
    categories = [
        ("correct", "Correct", "correct_count", "correct_share", "#5F998E"),
        (
            "compatible",
            "Compatible",
            "compatible_count",
    ...
    for category_index, (_, label, count_key, share_key, color) in enumerate(categories):
        ...
        for index, (bar, count) in enumerate(zip(bars, counts, strict=False)):
            ...
            text_color = (
                "white"
                if label in {"Correct", "Wrong"} and not is_small_top_segment
                else HOUSE_COLORS["black"]
            )
            ...
            ax.text(
                ...
                fontsize=8,
                color=text_color,
                fontweight="bold" if label in {"Correct", "Wrong"} else "normal",
                linespacing=1.05,
            )
...
def main() -> None:
    ...
    category_rows = analysis_rows
    if args.category_source is not None:
        category_rows = [
            row
            for row in normalize_rows(read_csv_rows(args.category_source))
            if row["model"] in set(models)
        ]
    ...
    for path in [*main_half_paths, *example_paths]:
        print(f"  {path}")
