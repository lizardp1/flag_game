...
from empirical_panel_helpers import (
...
def draw_empirical_ab_standalone() -> tuple[Path, Path, Path]:
    """Render the final empirical A/B panel without the retired mechanism panels."""
    setup_mechanism_sans_style()
    fig, axes = plt.subplots(2, 1, figsize=(3.75, 5.15), constrained_layout=False)
    population = draw_empirical_ab_panels(
        axes[0],
        {"failures": axes[1]},
        endpoint_mode="failure_decomposition",
    )
    empirical_panel_title(axes[0], "a) Population-size sweep")
    empirical_panel_title(axes[1], "b) Failure decomposition by N")
    fig.subplots_adjust(left=0.19, right=0.97, top=0.965, bottom=0.105, hspace=0.48)

    stem = "flag_game_pairwise_n_scaling_empirical_ab_failure_decomposition"
