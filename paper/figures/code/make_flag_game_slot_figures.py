...
if "MPLCONFIGDIR" not in os.environ:
    mpl_config_dir = Path("/tmp/flag-game-mplconfig")
    mpl_config_dir.mkdir(parents=True, exist_ok=True)
    os.environ["MPLCONFIGDIR"] = str(mpl_config_dir)
if "XDG_CACHE_HOME" not in os.environ:
    xdg_cache_dir = Path("/tmp/flag-game-cache")
...
BROADCAST_ALPHA_RESULT_ROOT = REPO_ROOT / "results" / "flag_game_broadcast" / "alpha_mix_count_sweep_triangle28_N8_seeds5"
...
SLOT_STEMS = {
    ...
    "protocol_all_gpt4o": "flag_game_slot_protocol_all_gpt4o_observers_ciq",
}
...
def plot_protocol_all_gpt4o_observers() -> list[Path]:
    rows = read_csv(PROTOCOL_CSV)
    specs = [
        ("Broadcast", "broadcast", "all_gpt_4o", PURPLE),
        ("Pairwise", "pairwise", "all_gpt_4o", ORANGE),
        ("4o mgr", "org", "all_gpt_4o", TEAL_LIGHT),
        ("5.4 mgr", "org", "manager_gpt_5_4_observers_gpt_4o", TEAL),
    ]
    labels: list[str] = []
    values: list[float] = []
    sems: list[float] = []
    ...
    ax.set_title("All GPT-4o observer agents", loc="left", fontweight="normal")
    style_axis(ax, y_min=0.35, y_max=0.68)
    ax.spines["bottom"].set_visible(False)
    ax.tick_params(axis="x", length=0)
    fig.subplots_adjust(left=0.17, right=0.985, top=0.965, bottom=0.24)
    return save(fig, SLOT_STEMS["protocol_all_gpt4o"])
...
def main() -> None:
    ...
    outputs.extend(plot_protocol_all_gpt4o_observers())
