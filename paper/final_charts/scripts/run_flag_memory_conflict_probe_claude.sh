...
# Run the full private-vs-social-evidence memory-conflict probe on Claude.
#
# Defaults match the existing pilot grid:
...
python scripts/run_flag_memory_conflict_probe.py \
  --out "${OUT:-results/flag_game/memory_conflict_probe_claude}" \
  --backend anthropic \
  --model "${HAIKU_MODEL:-claude-haiku-4-5-20251001}" \
  --model "${SONNET_MODEL:-claude-sonnet-4-6}" \
  --m 1 \
  --m 3 \
